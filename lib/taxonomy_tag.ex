...
defmodule Bonfire.TaxonomySeeder.TaxonomyTag do
  ...
  schema "taxonomy_tag" do
    ...
    # field(:pointer_id, Needle.UID) # optional pointer ID for the tag (only needed once a tage is actually used)
    # belongs_to(:pointer, Pointer, references: :pointer_id, type: Needle.UID) # optional pointer ID for the tag (only needed once a tage is actually used)
